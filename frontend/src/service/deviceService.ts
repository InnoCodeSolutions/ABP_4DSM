...
export async function fetchDeviceMovement(
  deviceId: string,
  timeRange?: string // Added timeRange parameter
): Promise<{
  movement: Derivador[];
  averageSpeedKmh: number;
}> {
  try {
    ...
    const url = `${api}/devices/${deviceId}/movement${params.toString() ? '?' + params.toString() : ''}`;
    ...
    console.log(
      "fetchDeviceMovement: Fetched movement for deviceId:", deviceId,
      "timeRange:", timeRange || "none",
      "entries:", data.movement.length
    );
    ...
    return {
      ...
      movement: data.movement.map(pt => ({
        ...
        latitude: pt.latitude ?? 0, // Default to 0 if null
        longitude: pt.longitude ?? 0, // Default to 0 if null
    ...
    console.error(`fetchDeviceMovement: Error fetching movement for ${deviceId}:`, err);
