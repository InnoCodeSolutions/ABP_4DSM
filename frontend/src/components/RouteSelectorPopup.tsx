...
const RouteSelectorPopup: React.FC<RouteSelectorPopupProps> = ({
  ...
  const fetchRouteData = async (deviceId: string, timeRange?: string, isMaritime?: boolean): Promise<GeoJSONRoute | null> => {
    try {
      ...
      const response = await axios.get(`http://192.168.15.4:3000/api/routes/device/${deviceId}/route`, {
          ...
          <ScrollView style={styles.scrollContainer}>
            ...
            <View style={styles.section}>
              <View style={styles.switchContainer}>
                <Text style={styles.label}>Rota Marítima</Text>
                <Switch
                  value={isMaritime}
                  onValueChange={setIsMaritime}
                  trackColor={{ false: "#767577", true: "#3B82F6" }}
                  thumbColor={isMaritime ? "#fff" : "#f4f3f4"}
                />
              </View>
            </View>
...
const RouteSelectorPopup: React.FC<RouteSelectorPopupProps> = ({
  ...
  const fetchRouteData = async (deviceId: string, timeRange?: string, isMaritime?: boolean): Promise<GeoJSONRoute | null> => {
    try {
      ...
      console.log("Fetching route from:", `${API_URL}/api/routes/device/${deviceId}/route`);
      ...
      if (axios.isAxiosError(err)) {
        console.error("Axios error details:", err.response?.data);
        setError("Falha ao conectar com o servidor. Verifique sua conexão ou o endereço do servidor.");
      } else {
        setError("Erro inesperado ao carregar a rota.");
      }
      return null;
    }
  };
