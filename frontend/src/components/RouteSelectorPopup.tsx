...
const RouteSelectorPopup: React.FC<RouteSelectorPopupProps> = ({
  ...
  const [isMaritime, setIsMaritime] = useState<boolean>(false);
  ...
  const fetchRouteData = async (deviceId: string, timeRange?: string, isMaritime?: boolean): Promise<GeoJSONRoute | null> => {
    try {
      ...
      console.log("RouteSelectorPopup: Retrieved token:", token ? "Present" : "Missing");
      if (!token) {
      ...
      console.log("RouteSelectorPopup: Fetched route coordinates:", response.data?.features?.[0]?.geometry?.coordinates.length || "No coordinates");
      return response.data as GeoJSONRoute;
    } catch (err) {
      console.error("RouteSelectorPopup: Error fetching route data:", err);
      if (axios.isAxiosError(err)) {
        console.error("Axios error details:", err.response?.data || "No response data");
      }
      return null;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (period === "day") {
      ...
      if (startDate === endDate) {
        const startTimeValid = startTime && validateTime(startTime) ? startTime : "00:00";
        const endTimeValid = endTime && validateTime(endTime) ? endTime : "23:59";
        if (startTimeValid === endTimeValid) {
          start = `${startDate}T${startTimeValid}:00Z`;
          end = `${startDate}T${startTimeValid}:59Z`; // Intervalo de 1 minuto
        }
      }
      timeRange = `${start}/${end}`; // Usar / como separador, compatível com gpsDao
      console.log("Generated timeRange for range:", timeRange); // Depuração
    } else if (period === "all") {
      timeRange = undefined;
    }
    ...
    console.log("RouteSelectorPopup: Submitting route selection:", {
      ...
      timeRange,
      isMaritime,
      routeCoordinates: routeData?.features?.[0]?.geometry?.coordinates.length || 0,
    });
