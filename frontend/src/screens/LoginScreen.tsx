...
import { MaterialCommunityIcons } from "@expo/vector-icons"; // Use @expo/vector-icons for icons
...
const LoginScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.log("Resposta do login:", JSON.stringify(response, null, 2)); // Log detalhado
      let token = response?.token || response?.data?.token || response;
      ...
      try {
        ...
        Alert.alert("Erro", "Falha ao salvar o token. Tente novamente.");
        return;
      }
      ...
      console.error("Erro no login:", JSON.stringify(error, null, 2)); // Log detalhado
      if (error.response) {
        Alert.alert(
          "Erro no login",
...
const LoginScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      ...
      console.error("Erro no handlePasswordReset:", JSON.stringify(error, null, 2));
      const message =
        error.response?.status === 404
          ? "Serviço de redefinição de senha não encontrado. Verifique se o servidor está ativo."
          : error.response?.data?.message ||
