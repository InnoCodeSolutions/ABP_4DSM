...
const MapScreen: React.FC = () => {
  ...
  // Fetch device route and history with optional time range and maritime flag
  const loadDeviceRoute = async (
    ...
    try {
      ...
      console.log("MapScreen: Fetching route and history for deviceId:", deviceId, {
        timeRange,
        isMaritime,
      });
      ...
      console.log(
        ...
        route?.features?.[0]?.geometry?.coordinates.length || "No coordinates"
      );
      ...
      console.log(
        "MapScreen: Fetched history entries:",
        movement?.length || "No entries"
      );
      setDeviceHistory(movement || []);

      setSelectedDevice(deviceId);
    } catch (error) {
      console.error("MapScreen: Error fetching route or history:", error);
      setDeviceRoute(null);
      setDeviceHistory([]);
  ...
  console.log(
    "MapScreen: Rendering with deviceRoute coordinates:",
    deviceRoute?.features?.[0]?.geometry?.coordinates.length || 0,
    ...
    <MapScreenErrorBoundary>
      <View style={styles.container}>
        <CustomMapView
          markers={markers}
          style={styles.map}
          onMarkerPress={(deviceId: string) => loadDeviceRoute(deviceId)} // Load route and history on marker press
          route={deviceRoute ?? undefined}
          history={deviceHistory} // Pass history to CustomMapView
        />
        <TouchableOpacity
